...
**Why good**: REQUEST_CHANGES despite no code findings — CI failure is a merge blocker. Failed check uses `[❌ name](url)` format for scannable, clickable links. Root cause/fix split (`**Root cause**: ...` + `**Fix**: ...`) visually separates diagnosis from action. Investigation goes beyond CI logs: verified the cross-repo commit status and explains why it's unreachable. Actionable fix in a single sentence. Empty comments array since this is a project-level concern.
