...
⚙️ **CI**
[If checks failing: bulleted list of `[❌ name](url) (time ago)` + `**Root cause**: ...` diagnosis + `**Fix**: ...` action (no count line). For multiple failures, note whether they share a root cause or are independent. If pending: "⏳ N check(s) still running". If all pass or unavailable: omit.]
